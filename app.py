...
if not st.session_state.pipeline:
    ...
    if st.session_state.generating:
        ...
        # Progress bar
        progress_percent = (st.session_state.progress_step / 6) if st.session_state.progress_step > 0 else 0.01
        st.progress(progress_percent)
        
        # Status display
        col1, col2 = st.columns([1, 3])
        with col1:
            st.metric("Step", f"{st.session_state.progress_step}/6")
        with col2:
        ...
        with st.expander("📋 Detailed Timeline", expanded=True):
            ...
            for i, (emoji, title, description) in enumerate(steps, 1):
                ...
                elif i == st.session_state.progress_step:
